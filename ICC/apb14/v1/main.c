...
void gradiente_conjugado(Dados* dados_variaveis){
  ...
  memcpy(dados_variaveis->vetor_r, dados_variaveis->matriz_ATransB, tam * sizeof(double));
  ...
  for (i=0; i < dados_variaveis->MAX_iTERACOES; i++){
    ...
    mult_soma_vetor(dados_variaveis->vetor_x,xa,v,w,dados_variaveis->N);
    mult_soma_vetor(dados_variaveis->vetor_r,dados_variaveis->vetor_r,z,(-w),tam);
    matriz_multi_vetor(dados_variaveis->matriz_I,dados_variaveis->vetor_r,y,dados_variaveis->N);
    ...
    aux = vetores_produto(y,dados_variaveis->vetor_r,dados_variaveis->N);
    if(rp != 0){
      h = aux / rp;
    }else{
      h = aux;
    ...
    rp = aux;
    mult_soma_vetor(v,y,v,h,dados_variaveis->N);
  }
