...
def encrypt():
	...
	try:
		file_name=str(input("Digite o nome do arquivo para cifrar: "))
	except ValueError:
		print ("Nome invalido")

	cifrar_fd = open(file_name,"r")
	message = cifrar_fd.read().strip().encode("utf-8")

	
	''.join([i for i in message if i.isalpha()]).upper
	print(message)
